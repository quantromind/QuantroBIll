...
export const TopBar: React.FC = () => {
  ...
  const { user, tenant, activeOutlet, availableOutlets, setActiveOutlet, toggleDrawer } = useAuthStore();
  ...
  const filteredNavTabs = navTabs.filter((tab) => {
    ...
    if (activeRole === 'DeliveryBoy') {
      return tab.path === '/online-orders';
    }
    if (activeRole === 'Cashier' || activeRole === 'Captain') {
      return tab.path !== '/finance' && tab.path !== '/reports';
    }
    return true;
  });
      ...
      <div className="bg-[#f8fafc] border-b border-slate-200/80 px-4 py-1 flex items-center justify-between text-xs text-slate-500">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <div className="relative">
            <div
              className={`flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full border text-[11px] font-bold shadow-2xs ${currentRoleInfo.badge}`}
            >
